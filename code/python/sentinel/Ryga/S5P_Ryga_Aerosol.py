...
FOLDER     = r'.\sentinel_data\S5P_Aerosol_Ryga' # Paskutinę dalį pakeiskit pagal produktą
...
def download_period(args):
    ...
    riga_geometry = ee.Geometry.Point([lon, lat]).buffer(30000) #Priminimas: ~30 km spindulys 

    collection = ee.ImageCollection(PRODUCT) \
        .filterDate(date_start, date_end) \
        .filterBounds(riga_geometry) \
        .map(lambda image: check_valid(image, riga_geometry)) \
